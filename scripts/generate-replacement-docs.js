...
const BASE =
  "https://raw.githubusercontent.com/outslept/module-replacements/refs/heads/transport-docs/";
...
function hasCommentedFrontmatter(md) {
  const lines = LF(md).split("\n");
  return (
    lines.length >= 5 &&
    lines[0].trim() === "<!--" &&
...
const annotateSuffix = (lang, mark) =>
  lang === "bash" ? ` # [!code ${mark}]` : ` // [!code ${mark}]`;

function dropMarkerKeepIndent(line) {
  if (!line) return line;
  const c0 = line[0];
  if (c0 !== "-" && c0 !== "+") return line;
  const c1 = line[1] ?? "";
  const c2 = line[2] ?? "";
  if (c1 === " ") return c2 === " " ? line.slice(1) : line.slice(2);
  if (c1 === "\t") return line.slice(1);
  return line.slice(1);
}
...
async function main() {
  ...
  for (const rel of files) {
    ...
    if (!hasCommentedFrontmatter(raw)) continue;
    const out = transformDiffFencesOnly(unwrapCommentedFrontmatter(raw));
